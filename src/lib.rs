...
use cocoa::appkit::{NSApp, NSApplication,
					...
					NSBackingStoreBuffered, NSImage, NSMenu,
					NSMenuItem, NSRunningApplication,
					NSSquareStatusItemLength, NSStatusBar, NSWindow, NSWindowStyleMask};
use cocoa::appkit::NSButton;
use cocoa::appkit::NSStatusItem;
use cocoa::base::{id, nil, NO, selector};
use cocoa::foundation::{NSAutoreleasePool, NSData, NSPoint, NSProcessInfo, NSRect, NSSize, NSString};
use thiserror::Error;
...
impl OsxSystemTray {
	...
	fn init_tray() -> id {
		unsafe {
			NSStatusBar::systemStatusBar(nil).statusItemWithLength_
	...
	pub fn set_tray_icon_from_buffer(&mut self, buffer: &[u8]) -> Result<(), Error> {
		unsafe {
			...
			let new_size = NSSize::new(ICON_WIDTH, ICON_HEIGHT);
			let r: () = msg_send![nsimage, setSize:new_size];
			self.tray.clone().lock().unwrap().button()
				.setImage_(nsimage);
		};
		Ok(())
	}
